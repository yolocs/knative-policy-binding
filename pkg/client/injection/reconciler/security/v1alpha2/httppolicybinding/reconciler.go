...
import (
	...
	securityv1alpha2 "github.com/yolocs/knative-policy-binding/pkg/client/listers/security/v1alpha2"
	zap "go.uber.org/zap"
...
// controller reconciling v1alpha2.HTTPPolicyBinding.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha2.HTTPPolicyBinding. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha2.HTTPPolicyBinding) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha2.HTTPPolicyBinding.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha2.HTTPPolicyBinding. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha2.HTTPPolicyBinding) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1alpha2.HTTPPolicyBinding resources.
type reconcilerImpl struct {
	// Client is used to write back status updates.
	Client versioned.Interface

	// Listers index properties about resources
	Lister securityv1alpha2.HTTPPolicyBindingLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister securityv1alpha2.HTTPPolicyBindingLister, recorder record.EventRecorder, r Interface) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha2.HTTPPolicyBinding, desired *v1alpha2.HTTPPolicyBinding) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {
			existing, err = r.Client.SecurityV1alpha2().HTTPPolicyBindings(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.SecurityV1alpha2().HTTPPolicyBindings(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha2.HTTPPolicyBinding) error {
	...
	_, err = r.Client.SecurityV1alpha2().HTTPPolicyBindings(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha2.HTTPPolicyBinding) error {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha2.HTTPPolicyBinding, reconcileEvent reconciler.Event) error {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
